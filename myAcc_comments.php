<?php session_start();
    ...
    if(isset($_GET['deleteId'])){
        $cid= $_GET['deleteId'];
        $sql = "update comments set status=:status where id=:cid";
        $status = "Delete";
        $data = [
            'status' => $status,
            'cid' => $cid
...
<body>
    ...
    <div class="container-fluid body_start" style="margin-top: 5rem; margin-bottom: 6rem;">
            ...
            <div class="row">
                <div class="col-12 col-md-4 col-lg-3 mypf">
                    ...
                    <div class="mt-md-5 mx-md-2 mynav">
                        ...
                        <hr class="my-2"><a href="myAcc_comments.php" class="fw-bold">မှတ်ချက်များ <span style="font-size: 15pt; font-weight: 600; color: red;"><?php echo $count_comments;?></span> ခု</a>
                    </div>
                </div>
        ...
        <?php 
            $userid = $_SESSION['userId'];
            $status = "Active";
            $sql="select * from comments where status=:status and user_id=:userid";
            $inputdata = [
                'status' => $status,
                'userid' => $userid
            ];
            ...
            if($result->rowCount()){
            ?>
                <h3 class="mb-3 mb-md-5 noPosts">မှတ်ချက်များ</h3>
            <?php
                $data = array_reverse($result->fetchAll(PDO::FETCH_ASSOC));
                foreach($data as $row){
                    $qid = $row['ques_id'];
                    $sql1="select * from questions where id=:qid";
                    $inputdata1 = [
                        'qid' => $qid
                    ];
            ...
            ?>
                        <div class="row mydiv" onclick="document.getElementById('<?php echo $row['id'];?>').style.display='block'">
                            <div class="col-sm-12 col-md-4 col-lg-3 p-0">
                                <img src="images/<?php echo $data1['photo'];?>" alt="" class="postimg">
                            </div>
                            <div class="col-sm-12 col-md-8 col-lg-9 p-2">
                                <p class="text-justify text-break "><?php echo $row['comment'];?></p>
                            </div>
                            <div class="d-flex justify-content-end">
                                <button type="btn btn-danger" class="mybtn m-3"><a href="myAcc_comments.php?deleteId=<?php echo $row['id'];?>" onClick="return confirm('Are you sure to delete');">Delete</a></button>
                            </div>
                        </div>
            <!-- comment more details popup modal -->
                        ...
                        <div class="modal-body morecommentsBody">
                            ...
                            <div class="moreCom">
                                <div class="d-flex justify-content-start">
                                    <img src="images/<?php echo $data1['photo'];?>" alt="" width="100%" height="100%" style="text-align: center:">
                                </div>                        
                            ...
                            ?>      
                                    <h6>မေး</h6>                                    
                                    <p class="text-justify text-decoration-underline"><?php echo $data2['name'];?></p>
                                    <p class="text-justify"><?php echo $data1['title'];?></p>
                                </div>
                            </div>
            ...
            ?>
                <h3 class="px-2 px-md-5 noPosts text-danger">မှတ်ချက်များ မရှိပါ။</h3>
            <?php
            }
            ?> 
                </div>
