    ...
    if(isset($_POST['add'])){
        ...
        ($_FILES["image"]["type"]=="image/png")||($_FILES["image"]["type"] == "image/pjpeg")) && in_array($extension,$allowedExts)){ 
            if($_FILES["image"]["error"] > 0){
                ...
                $sql = "insert into bean_diseases(name, photo, type, infection, characteristics, cause_time, prevention, status) values (:name, :photo, :type, :infection, :characteristics, :cause_time, :prevention, :status)";
                if($type == 'ပိုးမွှား'){
                    $cause_time = '-';
                }
                    $data = [
