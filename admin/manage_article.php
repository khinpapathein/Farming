...
if (isset($_GET["pid"])) {
    ...
    if ($stmt->execute()) {
        ...
        header("refresh:0; url=manage_article.php");
        exit(); // Ensure no other output interferes with the redirect
    } else {
        $errorInfo = $stmt->errorInfo();
        echo "Error deleting article: " . $errorInfo[2];
    }
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4">
                        <h1 class="mt-4">Added Articles</h1>
                        <ol class="breadcrumb mb-4">
                            <li class="breadcrumb-item active">Added</li>
                            <!-- <li style="margin-left: 80%;"><button onclick="location.href='add_article.php'" >Add Article</button></li> -->
                        </ol>
                        <div class="d-flex justify-content-end mb-3 mx-3">
                            <button class="btn btn-success fw-bold py-2" type="button">
                                <a href="add_article.php" class="text-decoration-none text-white">Add</a>
                            </button>
                        </div>
                        <div class="card mb-4">
...
foreach ($booksData as $book) {
    ...
    echo '<img src="' . $book['image'] . '" alt="" width="110px" height="70px">';
    echo '</td>';
    echo '<td class="text-break text-truncate" style="max-width: 200px;">' . $book['title'] . '</td>';
    
    // Assuming 'content' refers to book content
    echo '<td class="text-break text-truncate" style="max-width: 100px;">' . $book['content'] . '</td>';
    // Assuming there is no corresponding 'time' for books
    
    echo '<td>';
    echo '<a href="article_detail.php?pid=' . $book['id'] . '" class="px-1 text-decoration-none">';
    echo '<i class="fas fa-edit"></i>';
    echo '</a>';
    echo '<a href="manage_article.php?pid=' . $book['id'] . '" onClick="return confirm(\'Are you sure to delete\');">';
