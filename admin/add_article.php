<?php 
    ...
    $sql = "INSERT INTO articles (title, image, content) VALUES ('$title', '$coverPath', '$content')";

    $stmt = $conn->prepare($sql);

    if ($stmt->execute()) {
        echo "<script>alert('article added successful');</script>";
        header("refresh:0; url=manage_article.php");
        exit(); // Ensure no other output interferes with the redirect
    } else {
        $errorInfo = $stmt->errorInfo();
        echo "Error adding article: " . $errorInfo[2];
    }
                ...
                <div class="container px-5">
                                ...
                                <div class="card border border-dark">
                                    <div class="card-header" style="background-color: #35694a;">
                                        <a href="manage_article.php" class="px-3 pt-2 text-decoration-underline text-white">back</a>
                                    </div>
                                    <div class="card-body">
                                        <form method="post" class="form-horizontal" name="addPost" class="form-horizontal"  enctype="multipart/form-data">
                                            ...
                                            <div class="row form-group py-2 py-md-3">
                                                ...
                                                <div class="col-sm-7">
                                                    <textarea name="content" class="form-control" id="" cols="30" rows="4" required></textarea>
                                                </div>
                                            </div>
