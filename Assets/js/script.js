$(document).ready(function () {
    ...
    function getCurrentWeather(loc) {
        ...
        weatherObj = {
            ...
            $('#currTemp').text("အပူချိန်: " + weatherObj.temp + " " +  "°F");
            // render the current search city humidity
            $('#currHum').text("စိုစွတ်မှု: " + weatherObj.humidity + "%");
            // render current city search wind speed
            $('#currWind').text("လေတိုက်နှုန်း: " + weatherObj.wind + " MPH");      
        ...
        $.ajax({
            ...
            let uviTitle = '<span>ခရမ်းလွန်ရောင်ခြည်အညွန်းကိန်း: </span>';
    ...
    function getWeatherForecast(loc) {
        ...
        $.ajax({
            ...
            let $colmx1 = $('<div class="col-6 col-md-4 col-lg-3 p-0">');
            let $cardBody = $('<div class="card-body px-2 px-md-3 py-3 forecast-card">');
            ...
            $tempMaxLi.text('အမြင့်ဆုံး အပူချိန်: ' + weatherArr[i].maxTemp + " °F");
            $tempMinLi.text('အနိမ့်ဆုံး အပူချိန်: ' + weatherArr[i].minTemp + " °F");
            $humLi.text('လေတိုက်နှုန်း: ' + weatherArr[i].humidity + "%");
              // append HTML
            $iconLi.append($iconI);
            $ul.append($iconLi);
            $ul.append($weathLi);         
